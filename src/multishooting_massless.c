...
Here we assume the control parameter is the BS amplitude. 
The control parameter can be easily changed to be the frequency or central gravitational scalar, if desired.
The code increments the given range for the amplitude and for each one of them computes a solution.

Run this programme with multishooting.par file!

Certain BS models are very challenging to compute, especially in the regions of the parameter space, where gradients of the control parameter become infinite.
This happens around turning points and can be mitigated by changing the control parameter. Especially, I find frequency a better control parameter 
than the amplitude. To improve on the speed of convergence you may want to experiment with perturbations added to the initial guesses.
Here we use 1e-14 by defaul, but other values [1e-12,1e-10] are all suitable choices. Further accuracy may be improved through a long double; email me for code, if interested.
*/
...
typedef struct model
  ...
  double        *absphigrav; //Absolute value of the gravitational scalar field
  } model;
